...
#     ~/.claude/.clean/<session>.alerts.json.
...
cc_alerts_file() { echo "${HOME}/.claude/.clean/${1}.alerts.json"; }
...
cc_ram_threshold_check() {
  ...
  while IFS= read -r p; do
    ...
    if [ "$is_over" = "true" ] && [ "$was_over" != "true" ]; then
      ...
      echo "⚠ claude-clean: pid $p ($cmd) crossed ${rss_mb} MB — consider 'claude-clean stash $p'"
    fi
  done <<< "$live_pids"
    ...
    while IFS= read -r p; do
      ...
      while IFS= read -r wanted; do
        ...
        if echo ",$ports," | grep -q ",${wanted},"; then
          echo "⚠ claude-clean: port $wanted is already held by session ${s:0:6} (pid $p) — your command may fail to bind"
        fi
      done <<< "$wanted_ports"
    done <<< "$pids"
